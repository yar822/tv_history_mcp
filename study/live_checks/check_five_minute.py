...
ROOT = (Path(__file__).resolve().parents[2] / 'data/live_checks') / '20260909_160042' / 'five_minute_check'
...
async def main():
    ...
    async with streamablehttp_client('http://127.0.0.1:8010/mcp') as (read, write, _):
        async with ClientSession(read, write, read_timeout_seconds=timedelta(seconds=240)) as session:
            ...
            for asset in ('RUS:MX1!', 'RUS:SI1!'):
                await call(session, 'asset_bars', {'asset':asset,'timeframe':'1h','count':3})
