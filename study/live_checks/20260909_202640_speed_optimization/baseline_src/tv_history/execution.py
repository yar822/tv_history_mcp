...
def data_quality(frame: pd.DataFrame, timeframe: str, asset: str, calendar=None) -> dict:
    ...
    # A weekend anywhere inside a hole does not establish a scheduled closure.
    from .coverage import scheduled_gap
    calendar = calendar or frame.attrs.get("completion_context", {}).get("calendar")
    scheduled = sum(scheduled_gap(calendar, timeframe, end - gap, end)
                    for end, gap in gap_rows.items())
    if gap_rows.empty or len(gap_rows) == scheduled:
        unexpected = False
