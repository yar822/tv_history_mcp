...
async def call(session, name, args):
    ...
    try:
        ...
        records.append(row)
        print(json.dumps({k:row[k] for k in ('tool','arguments','seconds','error','tail','status') if k in row}), flush=True)
        ...
        async with ClientSession(read, write, read_timeout_seconds=timedelta(seconds=240)) as session:
            ...
            for asset in ('RUS:SI1!', 'COMEX:GC1!', 'CME_MINI:NQ1!', 'ICEEUR:BRN1!', 'BITSTAMP:BTCUSD'):
                for tf in ('1h','4h','1D','1W'):
                    await call(session, 'asset_bars', {'asset':asset,'timeframe':tf,'count':5})
                await call(session,'asset_analysis',{'asset':asset, 'timeframe':'1h'})
                await call(session,'asset_chart',{'asset':asset, 'timeframe':'1h','sessions':2})
            await call(session,'asset_bars',{'asset':'RUS:MX1!', 'timeframe':'1h','count':5})
