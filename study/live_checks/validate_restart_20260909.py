...
root = cfg.data_root
out = root/'live_checks'/datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S_restart_validation')
...
def audit_startup():
    ...
    log = log[pd.to_datetime(log.input_timestamp,utc=True) >= backup_start]
    selected = log[log.bars_requested == cfg.initial_bars]
    ...
    for asset, (zone, profile) in assets.items():
        ...
        for tf in ('1h','4h','1D','1W'):
            rows = selected[(selected.asset == asset)&(selected.timeframe == tf)]
            check(len(rows) == 1 and rows.iloc[0].status == 'success', f'{asset}/{tf}: startup full batch')
            if len(rows):
                check(pd.Timestamp(calendar['built_at']) >= pd.Timestamp(rows.iloc[-1].input_timestamp), f'{asset}/{tf}: calendar built before download')
            raw = store.read(asset,tf)
            validate_frame(raw)
            check(not raw.empty, f'{asset}/{tf}: empty cache')
            coverage = read_json(parent/'coverage.json',{}).get(tf,{})
            check(not coverage.get('refresh_error'), f'{asset}/{tf}: persisted refresh failure')
            tf_snapshot[tf] = {'rows':len(raw),'first':str(raw.index.min()),'last':str(raw.index.max()),
                               'confirmed_gaps':sum(g['status']=='incomplete' for g in coverage.get('unresolved_gaps',[])),
                               'uncertain_gaps':sum(g['status']=='uncertain' for g in coverage.get('unresolved_gaps',[]))}
        snapshot['assets'][asset] = {'calendar':calendar,'timeframes':tf_snapshot}
