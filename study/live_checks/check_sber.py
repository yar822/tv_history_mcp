...
ROOT = (Path(__file__).resolve().parents[2] / 'data/live_checks') / (datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S') + '_SBER_initialization')
...
async def main():
    ...
    async with streamablehttp_client('http://127.0.0.1:8010/mcp') as (read, write, _):
        async with ClientSession(read, write, read_timeout_seconds=timedelta(seconds=240)) as session:
            ...
            await call(session, 'asset_bars', {'asset':'RUS:SBER', 'timeframe':'1h', 'count':3})
            await call(session, 'asset_bars', {'asset':'SBER:RUS', 'timeframe':'1h', 'count':3})
