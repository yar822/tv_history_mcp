...
SESSION_PROFILES = {
    "moex_futures": {"timezone": "Europe/Moscow", "evening_start": 18,
                     "tail_mode": "moex_daytime", "require_tail_volume": True},
    "cme_overnight": {"timezone": "America/Chicago", "evening_start": 16,
                      "tail_mode": "overnight", "require_tail_volume": False},
    "ice_brent": {"timezone": "Europe/London", "evening_start": 22,
                  "tail_mode": "daytime_or_overnight", "require_tail_volume": False},
    "utc_calendar": {"timezone": "UTC", "evening_start": None,
                     "tail_mode": "disabled", "require_tail_volume": True},
}


def profile_date(opened, profile):
    ...
    zone = SESSION_PROFILES[profile]["timezone"]
...
def served_profile(source, daily, four, hourly, cutoff, timeframe, profile):
    policy = SESSION_PROFILES[profile]
    ending_timezone = policy["timezone"]
    rejected = []
    def rule(opened):
        try:
            return profile_date(opened, profile)
        except ValueError:
    ...
    verified = {}
    for label, row in mapped.iterrows():
    ...
    if timeframe == "1D":
        view = mapped
        accepted = [verified[r.source_timestamp] for _, r in view.iterrows()]
    else:
        view = normalize_weekly(source, daily, four, cutoff, hourly, date_rule=rule, ending_timezone=ending_timezone, holiday_policy=policy)
        accepted = []
        ...
        for label, row in view.iterrows():
            raw = row.source_timestamp
            ...
            if ok:
                ok = all(verified[r.source_timestamp] for _, r in part.iterrows())
